...
const SELLER_DATA = {
    ...
    phone: "84919754",
    picture: "https://images.unsplash.com/photo-1560179707-f14e90ef3623?q=80&w=200&h=200&fit=crop",
...
export default function SellerDetails(props: SellerProps) {
    const { id, picture, dealershipDetails, isDealership, email, fullName, phone, statistics, verificationStatus, memberSince, acctVerified } = SELLER_DATA;
    ...
    return ( <main className="py-8">
                            ...
                            {acctVerified && (
                                <div className="flex items-center text-verified text-sm bg-verified/10 px-3 py-1 rounded-full">
                                    <Shield className="h-4 w-4 mr-1" />
                                     {isDealership ? 'Agencia Verificada' : 'Vendedor Verificado'}
                                </div>
                            )}
                        </div>
        ...
        <div className="grid grid-cols-1 gap-8">
                    ...
                    {Object.entries(verificationStatus).map(([key, value]) => (
                        ...
                        >
                            <div className="flex items-center gap-3">
                                {key === 'identification' && <Certificate className="h-5 w-5 text-primary" />}
                                {key === 'email' && <Mail className="h-5 w-5 text-primary" />}
                                {key === 'phone' && <Phone className="h-5 w-5 text-primary" />}
                                <span className="font-medium capitalize">
                      {VERIFICATIONS_TRANSLATIONS[key as keyof typeof VERIFICATIONS_TRANSLATIONS]}
                    </span>
                            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="bg-quaternary p-6 rounded-lg">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <h3 className="text-lg font-medium text-tertiary">Listados Totales</h3>
                        </div>
                        <p className="text-3xl font-bold text-primary">{statistics.totalListings.toLocaleString()}</p>
                    </div>
                    <div className="bg-quaternary p-6 rounded-lg">
                        <div className="flex items-center gap-3 mb-2">
                            <DollarSign className="h-6 w-6 text-primary" />
                            <h3 className="text-lg font-medium text-tertiary">Autos Vendidos</h3>
                        </div>
                        <p className="text-3xl font-bold text-primary">{statistics.totalSold.toLocaleString()}</p>
                    </div>
                    <div className="bg-quaternary p-6 rounded-lg">
                        <div className="flex items-center gap-3 mb-2">
                            <Eye className="h-6 w-6 text-primary" />
                            <h3 className="text-lg font-medium text-tertiary">Vistas Totales</h3>
                        </div>
                        <p className="text-3xl font-bold text-primary">{statistics.totalViews.toLocaleString()}</p>
                    </div>
                </div>
            </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
                ...
                <div className={`space-y-8 ${isDealership && dealershipDetails ? 'lg:col-span-3' : 'lg:col-span-4'}`}>
                    <section className="bg-white rounded-lg shadow-md p-6">
                        <h2 className="text-2xl font-bold text-tertiary mb-6">
                            Listados Activos
                        </h2>
                    ...
                    <div className="lg:col-span-1">
                        <section className="bg-white rounded-lg shadow-md p-6 sticky top-16">
                            ...
                            {dealershipDetails.locationImages.length > 0 && (
                                <div className="mb-6">
                                    <h3 className="font-medium text-tertiary mb-3">Nuestra Ubicación</h3>
                                    <div className="relative aspect-video rounded-lg overflow-hidden">
                                        <Carousel images={dealershipDetails?.locationImages} model={`${id}`} showDots={true} />
                                    </div>
                                </div>
                            )}
