...
import {useGetMakes} from "@/app/car-entry/services/getMakes";
import {FormSelect} from "@/components/Forms/Select/FormSelect";
import {useGetModels} from "@/app/car-entry/services/getModels";
import {useGetTrims} from "@/app/car-entry/services/getTrims";
import {generateYears} from "@/lib/years";
import {FormInput} from "@/components/Forms/Input/FormInput";
import {useCreateMutation} from "@/app/car-entry/services/postListing";
...
export const CreateCar = () => {
    const { mutateAsync: createListing, isPending, isSuccess } = useCreateMutation();
    const [progress, setProgress] = useState(0);
    const router = useRouter()


    const onSubmit = useCallback((data: CarFormProps) => {
        setProgress(15);
        setTimeout(() => {
        setProgress(25);
        }, 1000);
    ...
    if (isPending || isSuccess) {
        return <div className="h-96 w-full flex flex-col gap-2 justify-center items-center p-4">
...
export const CarForm = (props: FormProps) => {
    ...
    return <form onSubmit={handleSubmit} className="flex flex-col gap-2 px-4 pb-4">
        <Field
            name="makeId"
        ...
        <div className="flex flex-col-reverse md:flex-row gap-3 justify-end mt-6">
            <Button type="button" variant="outline" className="text-tertiary text-opacity-90 w-full">
                Cancelar
            </Button>
            <Button type="submit" className="bg-primary w-full" disabled={!valid || !dirty}>
                Guardar
            </Button>
        </div>
    </form>
