'use client';
import {Box, ChevronDown, ChevronUp, Gauge, Info, Ruler, Wine as Engine} from 'lucide-react';
import {useState} from "react";
import {FactorySpecification} from "@/lib/definitions";

const tooltips = {
    engine: {
        cubicCentimeters: "Cilindrada del motor en centímetros cúbicos (cc), indicando el volumen total de todos los cilindros",
    ...
    economy: {
        mpgCombine: "Consumo de combustible promedio en conducción combinada (ciudad y carretera)",
        mpgCity: "Consumo de combustible en condiciones de conducción urbana",
        mpgHighway: "Consumo de combustible en condiciones de conducción en carretera"
    },
    dimensions: {
...
return  (<section className="bg-white rounded-lg shadow-md p-6">
    <div className="flex items-center gap-2 mb-4">
        <h2 className="text-xl font-semibold text-tertiary">Especificaciones de fábrica</h2>
                        ...
                        {Object.entries(dimensions).map(([key, value]) => (
                            <div key={key} className="flex items-center gap-2">
                                <div className="group relative">
                                    <Info className="h-4 w-4 text-primary cursor-help" />
                                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-3 py-2 bg-tertiary text-white text-sm rounded-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all w-48">
                                        {tooltips.dimensions[key as keyof typeof tooltips.dimensions]}
                                    </div>
                                </div>
                                <div>
                                    <p className="text-sm text-tertiary capitalize">
