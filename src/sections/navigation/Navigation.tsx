...
export const Navigation = () => {
  ...
  // Open the navigation automatically when coming back from SSO
  useLayoutEffect(() => {
    const href = typeof window !== 'undefined' ? window.location.href : '';
    if (
      href.includes('sso-callback') ||
      (pathname && pathname.includes('sso-callback'))
    ) {
      setOpen(true);
    }
  }, [pathname]);
  ...
  useLayoutEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      [split7, split6, split5, split4, split3, split2, split1].forEach(
        (split) => {
        ...
        >
          ...
          <SignedOut>
            ...
            <div className="flex gap-4 items-center text-sm mt-10">
              <Link id="how-to" href="/how" onClick={handleClose}>
                COMO PUBLICAR?
              </Link>
              <Link id="contact-us" href="/contact" onClick={handleClose}>
                CONTACTANOS
              </Link>
            </div>
          </SignedOut>
          <SignedIn>
            ...
            <div
              id="car-entry"
              className="my-6 font-lilita-one text-xl cursor-pointer"
