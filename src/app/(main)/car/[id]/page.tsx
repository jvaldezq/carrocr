...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  console.log('Generating metadata for car with ID:', id);
    ...
    <main>
      ...
      <div className="max-w-screen-xl mx-auto px-2 py-6 flex flex-col gap-4">
        ...
        >
          ...
          <h2 className="text-2xl font-bold text-black mt-4 md:mt-0">
        ...
        <div className="flex flex-col gap-4 mb-16">
          <h4 className={tw('text-lg', 'font-semibold', 'text-black')}>
            Descripción general (AI)
          </h4>
          <p className="text-sm font-light">
            In production for over a decade, the Lamborghini Aventador has
            reached the end of its long and winding road. To celebrate the
            model&apos;s success, the 2022 Aventador comes only in new LP 780-4
            Ultimae specification, its sonorous 6.5-liter V12 engine now
            equipped with titanium intake valves to make the Ultimae the most
            powerful example of the supercar ever made. Given the mere
            10-horsepower increase over last year&apos;s Aventador SVJ (Super
            Veloce Jota). But among supercar owners, bragging rights count, and
            buyers who scoop up examples of the 2022 Aventador from the final
            run of 350 coupes and 250 convertibles will be able to tell anyone
            interested enough to listen that their limited-edition car has the
            most powerful naturally aspirated standard-production V12 in
            Lamborghini history.
          </p>
          ...
          {sellerComment && (
            <>
              <h4
                className={tw('text-lg', 'font-semibold', 'text-black', 'mt-2')}
              >
        ...
        >
          <div className="flex flex-col gap-3">
            <h4 className={tw('text-lg', 'font-semibold', 'text-black')}>
              Motor y potencia (Fábrica)
            </h4>
            {Object.entries(factorySpecifications?.engine || {}).map(
              ([key, value]) => (
                <div key={key} className="flex items-start gap-2">
                  <Tooltip
                    tooltipContent={
                      factoryToolTips.engine[
                        key as keyof typeof factoryToolTips.engine
                      ].description
                    }
                  >
                    <p className="text-sm font-light">
                      ...
                      <strong className="font-semibold pl-4">
                        {value}{' '}
                        {
                          factoryToolTips.engine[
                            key as keyof typeof factoryToolTips.engine
          ...
          <div className="flex flex-col gap-3">
            ...
            {Object.entries(factorySpecifications?.economy || {}).map(
              ([key, value]) => (
                <div key={key} className="flex items-start gap-2">
                  <Tooltip
                    tooltipContent={
                      factoryToolTips.economy[
                        key as keyof typeof factoryToolTips.economy
                      ].description
                    }
                  >
                    <p className="text-sm font-light">
          ...
          <div className="flex flex-col gap-3">
            ...
            {Object.entries(factorySpecifications?.dimensions || {}).map(
              ([key, value]) => (
                <div key={key} className="flex items-start gap-2">
                  <Tooltip
                    tooltipContent={
                      factoryToolTips.dimensions[
                        key as keyof typeof factoryToolTips.dimensions
                      ].description
                    }
                  >
                    <p className="text-sm font-light">
